...
class FrameWriteBufferTensor:
    ...
    def add(self, frame_path: str, frame: torch.Tensor):

    # Add frame path to the buffer
        self.frame_paths_buffer.append(frame_path)

    # Serialize tensor into bytes
        frame_bytes_io = BytesIO()
        torch.save(frame, frame_bytes_io)
        frame_bytes = frame_bytes_io.getbuffer()
        self.frame_bytes_buffer.append(frame_bytes)

    # Flush buffer to ZIP file if it reaches the buffer size
        if len(self.frame_paths_buffer) == self.buffer_size:
            self.flush()
