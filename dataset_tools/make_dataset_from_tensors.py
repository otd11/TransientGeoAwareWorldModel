...
# from dataset_tools.utils import FrameWriteBufferTensor, ParallelProgressBar, center_crop_and_resize
 
from utils import FrameWriteBufferTensor, ParallelProgressBar, center_crop_and_resize
 
# =====================================================================================================================
 
def save_video_clip_tensor(
    zipfile_path: Path, relative_dir: str, paths: list[str], height: int, width: int
) -> tuple[str, list[str]]:
 
    frame_names = []
    # frame_write_buffer = FrameWriteBufferTensor(zipfile_path, quality=100, subsample=0)
    frame_write_buffer = FrameWriteBufferTensor(zipfile_path, quality=95)
    ...
    for path in paths:
        ...
        frame = tensor # .numpy()  # (C, H, W) -> (H, W, C)
 
        # Adds frame to the frame write buffer
        frame_name = Path(path).with_suffix(".pt").name
...
def list_frame_paths_tensor(input_dir: str, wait_for_list_file: bool = False) -> list[tuple[str, list[str]]]:
    ...
    with tqdm.tqdm(desc=f"Listing frame paths") as progress_bar:
        while directories:
            ...
            for entry in os.scandir(directory):
                ...
                elif entry.name.lower().endswith(".pt"):
...
def make_dataset_tensor(
    ...
    frame_paths = list_frame_paths_tensor(input_dir, wait_for_list_file)
    ...
    with ParallelProgressBar(n_jobs=-1) as parallel:
        ...
        frame_paths = parallel(save_video_clip_tensor, parallel_args)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process directories of tensors (.pt) to dataset partitions.")
    parser.add_argument("input_dir", help="Path to input tensor directories.")
