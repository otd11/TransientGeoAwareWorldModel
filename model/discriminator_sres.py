...
class VideoDiscriminator(nn.Module):
    ...
    hr_height: int = 256
    hr_width: int = 256
    cond_sp_channels: int = 1  # Number of channels for spatial conditioning, set to 0 if not used.
    ...
    def __post_init__(self):
        ...
        img_channels = (2 * self.channels + self.cond_sp_channels) * self.seq_length
    ...
    def forward(self, lr_video: torch.Tensor, hr_video: torch.Tensor, cond_sp: torch.Tensor) -> torch.Tensor:
        ...
        assert cond_sp.dim() == 4, f"cond_sp should be 4D (N, C_sp, H, W), got {cond_sp.shape}"
        assert cond_sp.size(2) == lr_video.size(3) and cond_sp.size(3) == lr_video.size(4), \
            f"cond_sp spatial size must match upsampled video: {cond_sp.shape[2:]} vs {lr_video.shape[3:]}"
        assert cond_sp.size(1) == self.cond_sp_channels, \
            f"cond_sp channels ({cond_sp.size(1)}) must match cond_sp_channels ({self.cond_sp_channels})"


        # Expand static conditioning across time
        cond_sp_expanded = cond_sp.unsqueeze(2).expand(-1, -1, lr_video.size(2), -1, -1)

        # Concatenate LR, HR, and cond_sp along channel dimension
        videos = torch.cat((lr_video, hr_video, cond_sp_expanded), dim=1)
