...
@persistence.persistent_class
@dataclasses.dataclass(eq=False, repr=False)
class ToChannels(nn.Module):  # Renamed class
    latent_dim: int
    in_channels: int
    out_channels: int = 3 # Allows variable output dimensions
    activation_clamp: Optional[float] = 256.0
    magnitude_ema: bool = True
    use_float16: bool = False
    ...
    def __post_init__(self):
        ...
        # Affine transformation to convert latent vector to modulation parameters
        self.affine = FullyConnectedLayer(self.latent_dim, self.in_channels, bias_init=1.0)

        # Initialize weight and bias dynamically for the specified output channels
        self.weight = nn.Parameter(torch.randn(self.out_channels, self.in_channels, 1, 1, 1))
        self.bias = nn.Parameter(torch.zeros(self.out_channels))

        # Optional Magnitude Exponential Moving Average for normalization
        if self.magnitude_ema:
            self.input_magnitude_ema = MagnitudeEMA()
    ...
    def forward(
        ...
        # Process latent tensor into modulation parameters (style)
        latent = einops.rearrange(latent, "n c t -> (n t) c")
        style = self.affine(latent)
        style = einops.rearrange(style, "(n t) c -> n c t", t=in_seq_length)

        # Adjust input tensor's dtype
        dtype = dtype if dtype is not None else (torch.float16 if self.use_float16 and input.is_cuda else torch.float32)
        input = input.type(dtype)

        # Optionally scale input using Magnitude EMA
        input_gain = self.input_magnitude_ema(input, magnitude_ema_beta) if self.magnitude_ema else None

        # Perform temporal modulated convolution
        output = temporal_modulated_conv3d(input, self.weight, style, input_gain, demodulate=False)

        # Add bias and optionally clamp activation values
        bias = self.bias.type(output.dtype)
        output = bias_act_wrapper(output, bias, act="linear", clamp=self.activation_clamp)
        return output
...
class VideoGenerator(nn.Module):
    ...
    channels: int = 3 # 4 #
    cond_num_dim: int = 0  # Added conditioning dimension (numerical values and spatial information)
    cond_sp_dim: int = 0  # Added conditioning dimension (numerical values and spatial information)
    ...
    def __post_init__(self):
        ...
        # Calculate temporal_emb_dim_map
        self.temporal_emb_dim_map = self.temporal_emb_dim + self.cond_num_dim
        ...
        self.cond_scales = [[heights[0], widths[0]], [heights[0], widths[0]], [heights[1], widths[1]], [heights[1], widths[1]], [heights[2], widths[2]], [heights[2], widths[2]], [heights[2], widths[2]], [heights[3], widths[3]], [heights[3], widths[3]], [heights[4], widths[4]], [heights[4], widths[4]]]
        self.cond_scales_dims = [self.cond_sp_dim] * 11
        ...
        self.temporal_layers = nn.ModuleList([
            Synthesis3dResBlock(self.latent_w_dim, in_channels=512 + self.cond_scales_dims[0], out_channels=512, out_height=heights[0], out_width=widths[0], temporal_up=True, **t_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=512 + self.cond_scales_dims[1], out_channels=512, out_height=heights[1], out_width=widths[1], temporal_up=True, spatial_up=True, **t_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=512 + self.cond_scales_dims[2], out_channels=512, temporal_up=True, **t_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=512 + self.cond_scales_dims[3], out_channels=512, out_height=heights[2], out_width=widths[2], temporal_up=True, spatial_up=True, **t_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=512 + self.cond_scales_dims[4], out_channels=256, temporal_up=True, **t_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=256 + self.cond_scales_dims[5], out_channels=256, **t_kwargs),
        ])
        self.spatial_layers = nn.ModuleList([
            Synthesis3dResBlock(self.latent_w_dim, in_channels=256 + self.cond_scales_dims[6], out_channels=128, out_height=heights[3], out_width=widths[3], spatial_up=True, **s_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=128 + self.cond_scales_dims[7], out_channels=128, **s_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=128 + self.cond_scales_dims[8], out_channels=64, out_height=heights[4], out_width=widths[4], spatial_up=heights[4] != heights[3], **s_kwargs),
            Synthesis3dResBlock(self.latent_w_dim, in_channels=64 + self.cond_scales_dims[9], out_channels=64, out_height=self.out_height, out_width=self.out_width, **s_kwargs),
        ])

        # ToChannels allows flexible output channels in comparison to ToRGB. We do not rename the variable.
        self.to_rgb = ToChannels(self.latent_w_dim, in_channels=self.spatial_layers[-1].out_channels + self.cond_scales_dims[10] , out_channels=self.channels) 
        ...
        self.spatial_input = nn.Parameter(torch.randn(1, self.temporal_layers[0].in_channels-self.cond_scales_dims[0], 1, heights[0], widths[0]))
        self.temporal_emb = BlurredNoise(self.temporal_emb_dim, **self.embedding_kwargs)
        # Increase input dim of LatentMappingNetwork by cond_num_dim since cond is concatenated to temporal_emb.
        # Output dim stays the same since it is used to generate latent_w_dim.
        self.latent_mapping = LatentMappingNetwork(self.temporal_emb_dim_map, self.latent_w_dim, **self.mapping_kwargs)
        self.temporal_downsample_latent = TemporalKaiserDownsample()
        self.w_to_temp_input = FullyConnectedLayer(self.latent_w_dim, self.temporal_layers[0].in_channels-self.cond_scales_dims[0])
    ...
    def synthesize_video(
        ...
        misc.assert_shape(temporal_input, (None, self.temporal_layers[0].in_channels-self.cond_scales_dims[0], in_seq_length))
        ...
        if cond is not None:
            ...
            # create list of scaled binary masks
            masks = self.compute_scaled_masks(mask=mask)
        ...
        for layer, layer_seq_length in zip(self.temporal_layers, seq_lengths):
            ...
            if cond is not None:
                ...
                mask_expanded = masks[w_index].unsqueeze(2).expand(features.size(0), self.cond_sp_dim, features.size(2), features.size(3), features.size(4))


                features = torch.cat([features, mask_expanded], dim=1) # concatenate scaled mask to features
    ...
    ### add conditioning vector cond

    def forward(
        self,
        batch_size: int,
        seq_length: int,
        cond: Optional[dict[torch.Tensor]] = None,  # Conditioning vector is now a list
        magnitude_ema_beta: float = 1.0,
        generator_emb: Optional[torch.Generator] = None,
        dtype: Optional[torch.dtype] = None,
    ) -> Union[torch.Tensor, tuple[torch.Tensor, torch.Tensor]]:
        
        # Sample temporal embedding with conditioning
        temporal_emb = self.sample_temporal_emb(batch_size, seq_length, cond_num=cond["cond_num"], generator_emb=generator_emb)

        # Compute latent codes from the modified temporal embedding
        ...
        return self.synthesize_video(temporal_input, latent_ws, seq_length, magnitude_ema_beta, dtype, cond=cond)
    ...
    def sample_video_segments(
        ...
        if cond is not None:                       
            ...
            # create list of scaled binary masks
            masks = self.compute_scaled_masks(mask=cond["cond_sp"])
    ...
    def sample_temporal_input(
        ...
        temporal_input = torch.randn(
            batch_size,
            self.temporal_layers[0].in_channels-self.cond_scales_dims[0],
            input_seq_length,
            generator=generator,
            device=device,
        )
    ...
    def sample_temporal_emb(
        self, batch_size: int, 
        seq_length: int, 
        cond_num: Optional[torch.Tensor] = None,  
        generator_emb: Optional[torch.Generator] = None
    ) -> torch.Tensor:
        input_seq_length = self.compute_seq_lengths(seq_length)[0]
        temporal_emb_seq_length = input_seq_length * self.total_temporal_scale

        # Sample temporal embedding
        temporal_emb = self.temporal_emb(batch_size, temporal_emb_seq_length, generator_emb)


        if cond_num is not None:

            if cond_num.ndim == 1: # this occurs for batchsize = 1, i.e. 
                cond_num = cond_num.unsqueeze(0)  # Add batch dim 


            # Expand `cond` to match the temporal dimensions
            cond_expanded = cond_num.unsqueeze(-1).expand(-1, -1, temporal_emb.shape[2])  # Shape: (batch_size, c_dim, seq_length)

            if temporal_emb.shape[0] < cond_expanded.shape[0]:
                cond_expanded = cond_expanded[:temporal_emb.shape[0], ...]

            # Concatenate `cond` with the temporal embedding along the channel dimension
            temporal_emb = torch.cat([temporal_emb, cond_expanded], dim=1)  # New shape: (batch_size, in_channels + cond_num_dim, seq_length)
    ...
    def compute_latent_ws(self, temporal_emb: torch.Tensor, seq_length: int) -> torch.Tensor:
        ...
        misc.assert_shape(temporal_emb, (None, self.temporal_emb_dim_map, None))
    ...
    def scale_binary_mask(self, mask: torch.Tensor, new_width: int, new_height: int) -> torch.Tensor:
        """
        Scales a binary mask tensor to the specified dimensions.

        Parameters:
        mask (torch.Tensor): Input binary mask tensor with dimensions (b, c, n1, n2).
        new_width (int): Desired width of the output tensor.
        new_height (int): Desired height of the output tensor.

        Returns:
        torch.Tensor: Scaled binary mask tensor with dimensions (b, c, new_height, new_width).
        """

        # Use torch.nn.functional.interpolate to resize the mask
        scaled_mask = F.interpolate(mask, size=(new_height, new_width), mode='nearest')
        
        return scaled_mask
    
    
    def compute_scaled_masks(self, mask: torch.Tensor) -> list:
        """
        Computes scaled masks for each pair of dimensions in self.cond_scales.

        Parameters:
        mask (torch.Tensor): Input binary mask tensor with dimensions (1, n1, n2).

        Returns:
        list: List of scaled binary mask tensors.
        """
        scaled_masks = []
        for h, w in self.cond_scales:
            scaled_mask = self.scale_binary_mask(mask, w, h)
