...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        cond_sp_channels    =  1,           # Number of spatial conditioning channels.
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        #### added for spatial conditioning
        self.cond_sp_channels = cond_sp_channels
        self.cond_channels = self.img_channels * (2 * self.cond_context + 1) + self.cond_sp_channels
    ...
    def forward(self, z, cond, cond_sp, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ...
        conds = self.prep_cond(cond)
        conds_sp = self.prep_cond_sp(cond_sp, out_seq_length)
        conds_all = [torch.cat([c, csp], dim=1) for c, csp in zip(conds, conds_sp)]

        # Synthesize
        img = self.synthesis(ws, conds_all, update_emas=update_emas, **synthesis_kwargs)
    ...
    def prep_cond_sp(self, cond_sp, out_seq_length):
        # cond_sp: [batch, channels, height_sp, width_sp]
        cond_sp = cond_sp.unsqueeze(2).repeat(1, 1, out_seq_length, 1, 1)  # [batch, channels, frames, height_sp, width_sp]
        cond_sp = einops.rearrange(cond_sp, "n c t h w -> (n t) c h w")
        conds_sp = []
        for name in self.synthesis.layer_names:
            in_width, in_height = getattr(self.synthesis, name).in_size
            layer_cond_sp = cond_sp
            x0 = max(0, (layer_cond_sp.size(3) - in_width) // 2)
            y0 = max(0, (layer_cond_sp.size(2) - in_height) // 2)
            x1 = x0 + in_width
            y1 = y0 + in_height
            layer_cond_sp = layer_cond_sp[:, :, y0:y1, x0:x1]
            px0 = (in_width - layer_cond_sp.size(3)) // 2
            px1 = (in_width - layer_cond_sp.size(3) + 1) // 2
            py0 = (in_height - layer_cond_sp.size(2)) // 2
            py1 = (in_height - layer_cond_sp.size(2) + 1) // 2
            layer_cond_sp = torch.nn.functional.pad(layer_cond_sp, (px0, px1, py0, py1), mode="replicate")
            conds_sp.append(layer_cond_sp)
        return conds_sp
...
class VideoGenerator(torch.nn.Module):
    ...
    num_fp16_res: int = 4
    channels: int = 3
    cond_sp_channels: int = 1  # Number of spatial conditioning channels.
    ...
    def __post_init__(self):
        ...
        self.SG3 = Generator(
            ...
            cond_sp_channels=self.cond_sp_channels,
        )
    ...
    def forward(
        self, lr_video: torch.Tensor, cond_sp: torch.Tensor, generator_z: Optional[torch.Generator] = None, magnitude_ema_beta: float = 1.0
        ...
        return self.SG3(latent_z, lr_video, cond_sp, update_emas=update_emas)

    def sample_video_segments(
        self, lr_video: torch.Tensor, cond_sp: torch.Tensor, segment_length: int = 8, generator_z: Optional[torch.Generator] = None
        ...
        for lr_video_segment in lr_video_segments.unbind(dim=-1):
            hr_video_segment = self.SG3(latent_z, lr_video_segment, cond_sp)
