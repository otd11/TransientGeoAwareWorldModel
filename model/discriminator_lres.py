...
class DiscriminatorEpilogue(nn.Module):
    ...
    cond_num_dim: int = 0  # Added conditioning dimension
    ...
    def __post_init__(self, **_kwargs):
        ...
        for index in range(self.num_linear_layers):
            ...
            if index == 0:
                ...
                in_channels += self.cond_num_dim  # Increase input size for conditioning
            else:
                in_channels = self.channels
    ...
    def forward(self, input: torch.Tensor, cond_num: Optional[list[torch.Tensor]] = None) -> torch.Tensor:
        """
        Forward pass of the epilogue.

        Args:
            input: Tensor of shape (N, C, T, H, W) - extracted video features.
            cond_num: Optional Tensor of shape (N, d) - conditioning vector.

        Returns:
            Tensor of shape (N, 1) - discriminator logits.
        """
        misc.assert_shape(input, (None, self.in_channels, self.in_seq_length, self.in_res, self.in_res))

        # Flatten spatial dimensions and apply 1D convolutions over time
        ...
        # Concatenate conditioning vector if provided
        if cond_num is not None:


            if cond_num.ndim == 1: # this occurs for batchsize = 1, i.e. (c_dim, seq_length)
                cond_num  = cond_num.unsqueeze(0)  # Add batch dim 


            cond_num = cond_num.to(features.dtype)  # Ensure dtype consistency
            features = torch.cat([features, cond_num], dim=1)  # Concatenate along feature axis
...
class VideoDiscriminator(nn.Module):
    ...
    def __post_init__(self):
        ...
        self.blocks = nn.ModuleList([
            DiscriminatorBlock(32 , 64, self.channels + self.cond_sp_dim, spatial_ksize=self.spatial_ksize, temporal_ksize=1, temporal_down=False, spatial_down=self.max_edge > 32, use_fp16=self.num_fp16_res > 0, conv_clamp=self.conv_clamp),
        ...
        self.epilogue = DiscriminatorEpilogue(
            ...
            cond_num_dim=self.cond_num_dim,  # Pass cond_num_dim
            **self.epilogue_kwargs,
        )

    def forward(self, videos: torch.Tensor, cond: Optional[dict[torch.Tensor]] = None) -> torch.Tensor:
            """
            Forward pass of the video discriminator.

            Args:
                videos: Tensor of shape (N, C, T, H, W) - input video frames.
                cond: Optional Tensor of shape (N, d) - conditioning vector.
            ...
            if cond is not None:
                ...
                if cond["cond_sp"].ndim == 3:  
                     cond["cond_sp"] = cond["cond_sp"].unsqueeze(0)  

                # Step 1: Add a temporal dimension
                cond_expanded = cond["cond_sp"].unsqueeze(2)  # Shape: (b, d, 1, h, w)

                # Step 2: Expand to match the features tensor
                cond_expanded = cond_expanded.expand(features.size(0), cond["cond_sp"].size(1), features.size(2), features.size(3), features.size(4))  # Shape: (b, d, s, h, w)

                features = torch.cat([features, cond_expanded], dim=1)
            ...
            cond_num = cond["cond_num"] if cond["cond_num"] is not None else None  # Extract cond_num

            logits = self.epilogue(features, cond_num=cond_num)  # Pass cond (can be None)
            return logits
