...
class LowResVideoGAN:
    ...
    def update_G(self, batch: int, cond: Optional[dict[torch.Tensor]] = None):
        ...
        for _ in range(self.G_grad_accum):
            ...
            video = self.G(
                ...
                cond=cond if cond is not None else None  # Pass `cond` if available
            )
    ...
    def update_D(self, real_video: torch.Tensor, cond: Optional[dict[torch.Tensor]] = None):
        ...
        fake_video = self.G(
            ...
            cond=cond if cond is not None else None
        )
        ...
        for fake_video_chunk, real_video_chunk in zip(
            ...
            fake_logits = self.run_D(fake_video_chunk, cond)
            fake_loss = F.softplus(fake_logits)
            fake_loss.mean().backward()

            real_logits = self.run_D(real_video_chunk, cond)
    ...
    def update_r1(self, video: torch.Tensor, cond: Optional[dict[torch.Tensor]] = None, gain: float = 1.0):
        ...
        cond_sp_chunks = cond["cond_sp"].chunk(self.D_grad_accum) if cond and "cond_sp" in cond and cond["cond_sp"] is not None else [None] * self.D_grad_accum
        cond_num_chunks = cond["cond_num"].chunk(self.D_grad_accum) if cond and "cond_num" in cond and cond["cond_num"] is not None else [None] * self.D_grad_accum

        for i, video_chunk in enumerate(video.chunk(self.D_grad_accum)):
            video_chunk = video_chunk.detach().requires_grad_(True)
            cond_sp_chunk = cond_sp_chunks[i]
            cond_num_chunk = cond_num_chunks[i]

            # Only add to cond_chunk if not None
            cond_chunk = {}
            if cond_sp_chunk is not None:
                cond_sp_chunk = cond_sp_chunk.detach().requires_grad_(True)
            cond_chunk["cond_sp"] = cond_sp_chunk
            if cond_num_chunk is not None:
                cond_num_chunk = cond_num_chunk.detach().requires_grad_(True)
            cond_chunk["cond_num"] = cond_num_chunk

            logits = self.run_D(video_chunk, cond_chunk if cond_chunk else None)

            # Build r1_inputs list
            r1_inputs = [video_chunk]
            if cond_sp_chunk is not None:
                r1_inputs.append(cond_sp_chunk)
            if cond_num_chunk is not None:
                r1_inputs.append(cond_num_chunk)

            r1_grads = torch.autograd.grad(outputs=[logits.sum()], inputs=r1_inputs, create_graph=True)
            ...
            training_stats.report("loss/r1_loss", r1_loss)
    ...
    def run_D(self, video: torch.Tensor, cond: Optional[Dict[str, Any]] = None) -> torch.Tensor:
        misc.assert_shape(video, (None, self.channels, self.seq_length, self.height, self.width))
        ...
        logits = self.D(video, cond=cond if cond is not None else None)  # Pass `cond` to `D`
        return logits
