...
class SuperResVideoGAN:
    ...
    def update_G(self, lr_video: torch.Tensor, cond_sp: torch.Tensor):
        ...
        for lr_video_chunk in lr_video.chunk(self.G_grad_accum):
            hr_video_chunk = self.G(lr_video_chunk, cond_sp)
            lr_video_chunk = self.crop_to_seq_length(lr_video_chunk)
            logits = self.run_D(lr_video_chunk, hr_video_chunk, cond_sp)
    ...
    def update_D(self, fake_lr_video: torch.Tensor, real_lr_video: torch.Tensor, real_hr_video: torch.Tensor, cond_sp: torch.Tensor):
        ...
        fake_hr_video = self.G(fake_lr_video, cond_sp, magnitude_ema_beta=self.G_magnitude_ema_beta)
        ...
        for fake_lr_video_chunk, fake_hr_video_chunk, real_lr_video_chunk, real_hr_video_chunk in zip(
            ...
            fake_logits = self.run_D(fake_lr_video_chunk, fake_hr_video_chunk, cond_sp)
            fake_loss = F.softplus(fake_logits)
            fake_loss.mean().backward()

            real_logits = self.run_D(real_lr_video_chunk, real_hr_video_chunk, cond_sp)
    ...
    def update_r1(self, lr_video: torch.Tensor, hr_video: torch.Tensor, cond_sp: torch.Tensor, gain: float = 1.0):
    ...
    def run_D(self, lr_video: torch.Tensor, hr_video: torch.Tensor, cond_sp: torch.Tensor) -> torch.Tensor:
        misc.assert_shape(lr_video, (None, self.channels, self.seq_length, self.lr_height, self.lr_width))
        misc.assert_shape(hr_video, (None, self.channels, self.seq_length, self.hr_height, self.hr_width))
        ...
        logits = self.D(lr_video, hr_video, cond_sp)
        return logits
